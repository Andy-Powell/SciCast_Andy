# if ordered (opq==2), length(tmp1)==2 -> "binary ordered (one Q?) or scaled (usually)"
# if non-ordered (orq==1), length(tmp1)==2 -> "binary"; length(temp1)>2 ->  multi 
#
# RPS
...
#rsq <- sort(levels(factor(rsq[cls==1 &nsmfq>9 &nsmdq>9]))) 
...
#for (q in 6:6) {                    #tetsting only
  ...
  w <- which(tat%in%tat[tat>=astart &qit==rsq[q] &asqt<0 &mdt==1 &cls==1 &nsmfq>9 &nsmdq>9])		# For Steve Stratman, do not include conditional forecasts.
  #w <- which(tat%in%tat[tat>=astart &qit==rsq[q] &asqt<0 &cls==1 &nsmfq>9 &nsmdq>9])    ### cls & mdt redundant? ##
  #w <- which(tat%in%tat[tat>=astart &qit==rsq[q] &asqt<0])           # For all qusestions
...
#  
  ...
  if (clq[qiq==rsq[q]]=="ordered multinomial") { 
    print("ordered multinomial")
    acttop <-rep(0,length(tmp1)-1)                      # length(temp1)-1 -> number of queston choices derived from resoluton values (rvq)
    
  
    for (o in 1:(length(tmp1)-1)) {                     # for all resoluton values....
      acttop[o] <- 2*(o/length(tmp1)-sum(tmp1[1:o]))^2  #
    }
    # svt uses the binned safe mode forecasts
    # instead if temp2, temp0 is matrix of svt -- ##### svt needs to have values for all trades, not just same-mode(?), binary(?)
    ...
    if (lt>1) {
      ...
      for (t in 1:ltStop) {                            # t is the current forecast ####### why stp?  Wrong t
        ...
        # Same as previosu section except temp2 repalced by temp3
      ...
      qstnDayAvgFrcstULinOP[q,d2] <- tmp3[2]
...
#  }  
  ...
  # unordered multinomial questions
  #    if (orq[qiq==rsq[q]]==1) {
  if (clq[qiq==rsq[q]]=="unordered multinomial") {
    print("unordered multinomial")
    ...
    if (lt>1) {
      ...
      for (t in 1:ltStop) {                            # t is the current forecast ####### why stp?  Wrong t
        ...
        if (t<2) {                              # first forecast
          ...
          tmp3[2] <- sum(tmp0[or[1:t]]*weight[or[1:t]])/sum(weight[or[1:t]])
          tmp3[1] <- 1-tmp3[2]
        ...
        stv[t+1] <- tmp3[2]   #### ???? #####
        #       actop[t+1]<-sum( (tmp3[t]-tmp1)^2 )  # sum of squares (cut tmp3 to length(tmp1))  Why do this for each timestep?
        #       actop[t+1]<-2*(tmp3[2]-tmp1[2])^2        # sum of squares (binary only)
        actop[t]<- sum((tmp3-tmp1)^2)       # sum of squares (binary ULinOP only)
        pocotop[t+1] <- mean(tmp3[b])
        if (mean(which(tmp3==max(tmp3)))%in%b) {hittop[t+1] <- 1}
      }
      qstnDayAvgFrcstULinOP[q,d2] <- tmp3[2]
