...
length(tat)
tat[tat<expChange1] <- NA
length(tat)
goodB <-complete.cases(tat)
sum(!goodB)

tat<-tat[goodB];tit<-tit[goodB]; pit<-pit[goodB]; qit<-qit[goodB]; nvt<-nvt[goodB]; ovt<-ovt[goodB]; ast<-ast[goodB]; apot<-apot[goodB]
cit<-cit[goodB]; rst<-rst[goodB]; mdt<-mdt[goodB]; asqt<-asqt[goodB]; asot<-asot[goodB]
length(tat)
...
acun <- acop <- nfqu <- acquCon <- rep(2,length(rsqExpB)); pocosCon <- pocou <- pocoop <- hitCon <- hitop <- rep(0,length(rsqExpB)); ra <- rep(tstart,length(rsqExpB))#; ra <- rep(as.POSIXct("2013-11-25 00:00:00 EST"),length(rsqExpB))
base <- tstart-28*24*60*60
for (q in 1:length(rsqExpB)) {
#for (q in 85:85) {
 ra[q] <- raq[qiq==rsqExpB[q]]
# Uses as question start date the first day on which there  was a valid safe mode forecast placed!
 ...
 w <- which(tat%in%tat[tat>=expStart &tat<expStop &qit==rsqExpB[q] &asqt%in%c(-1,rsqExpB) &asot==roqat])
 time <- c(tat[w],ra[q]); or <- order(time); time <- time[or]
 lt <- length(time); nfqu[q] <- lt-1
  tmp1 <- as.double(strsplit(strsplit(strsplit(as.character(rvq[qiq==rsqExpB[q]]),"[",fixed=T)[[1]][2],"]",fixed=T)[[1]],",")[[1]])
  ...
  if (clq[qiq==rsqExpB[q]]=="ordered multinomial") {
    ...
    #print (c(rsqExpB[q],"ordered",tmp1,"-", act[t+1]))
  }
  
  if (clq[qiq==rsqExpB[q]]=="unordered multinomial") {
    ...
    #print (c(rsqExpB[q],"unordered",tmp1,"-", act[t+1]))
  }
  
  
  if (clq[qiq==rsqExpB[q]]=="binary") {
    act[1] <- acun[q] <- (length(tmp1)-1)*(1/length(tmp1))^2+(1-1/length(tmp1))^2
    if (lt>1) {
      for (t in 1:(lt-1)) {
        tmp2 <- as.double(strsplit(as.vector(nvt[w])[or[t]],",")[[1]])
    ...
    #print (c(rsqExpB[q],"binary",tmp1,"-", act[t+1]))
  }
  
  if (clq[qiq==rsqExpB[q]]=="scaled") {
    act[1] <- acun[q] <- (length(tmp1)-1)*(1/length(tmp1))^2+(1-1/length(tmp1))^2
    if (lt>1) {
      for (t in 1:(lt-1)) {
        tmp2 <- as.double(strsplit(as.vector(nvt[w])[or[t]],",")[[1]])
    ...
    #print (c(rsqExpB[q],"scaled",tmp1,"-", act[t+1]))
  }

  if (clq[qiq==rsqExpB[q]]=="shares") {
    act[1] <- acun[q] <- (length(tmp1)-1)*(1/length(tmp1))^2+(1-1/length(tmp1))^2
    if (lt>1) {
      for (t in 1:(lt-1)) {
        tmp2 <- as.double(strsplit(as.vector(nvt[w])[or[t]],",")[[1]])
    ...
    #print (c(rsqExpB[q],"shares",tmp1,"-", act[t]))
  }
