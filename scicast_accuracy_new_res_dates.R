...
strtail <- function(s,n=1) {                   #what is n?
    if(n<0) 
        substring(s,1-n) 
    else 
...
purify_pr <- function(data) {
    ...
    for (i in 1:length(pip)) {
        for (g in 1:20) {
            if (strtail(grp[i,g],5)=="Admin") {    # looking at the last 5 characters in each string to see if it is admin
                cap[i] <- NA                       # cap is a vestor of created_at dates or NA for admin users
                adi <- c(adi,pip[i])
            }
            if (grp[i,g]=="Internal") {            # igrp is a vector with 1s for user_id that have internal for group and 0s otherwise.  Used to keep 
              igrp[i] <- 1                         # track of internal accounts!  When is this used?# Keeping but noting internal accounts
            }
        }
    }
    adi <- unique(adi)
...
summary(data)
igrp <- data$igrp;
#GrpsList <- data$GrpsList
adi <- data$adi
good <- complete.cases(data$cap)
sum(!good)                                        # How many are not good?
cap<-data$cap[good];
#CreatdAt <- data$CreatdAt[good]
pus<-data$pus[good];
#UsrNamChar <- data$UsrNamChar[good]
pip<-data$pip[good];
#UsrId <- data$UsrId[good]
grp<-data$grp[good,];
#Grps <- data$GrpAry[good,]
rip<-data$rip[good]
#RefId <- data$RefId[good]
...
raq <- as.character(qn$resolution_at)                         ##(qn$provisional_settled_at) overriden below
...
# Removing forecasts that occurred after event resolution.
for (t in 1:length(tat)) {
...
##############################################
## Note: Binary and ordered means continuous; it makes no difference to Brier, but it does make a difference on "poco" and "hit".
...
rsq <- levels(factor(qiq[saq<=Sys.time()&caq>tstart&ctq!="Study 2.1"&ctq!="Study 2.1,Study 2.1"&vldq==1]))
frc <- numeric();
rqb <- length(rsq)
