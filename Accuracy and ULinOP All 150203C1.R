...
sum(!good)     												# How many are not good?
tat<-tat[good]; pit<-pit[good]; qit<-qit[good]; nvt<-nvt[good]; ovt<-ovt[good]; ast<-ast[good]
apot<-apot[good]; tit<-tit[good]; cit<-cit[good]; rst<-rst[good]; mdt<-mdt[good]
...
frc <- numeric()
for (q in 1:length(rsq)) {frc[q] <- length(tat[qit==rsq[q]&pit%in%pip[igrp==0] &mdt==1])}  				# Checking the total forecasts on each question included for analysis.
rsq <- rsq[frc>2]									
...
#rsq <- sort(c(levels(factor(qiq[saq<=tstop &caq>tstart &"Public"%in%gq &vldq==1 &mdt==1])),"232","648")) 
...
mtext(paste("ULinOP Forecasts, mean = ",round(acopm,3),sep=""),col=rgb(0,0,1,0.6), outer=T,side=3,line=-12.5,cex=0.75,font=1)
