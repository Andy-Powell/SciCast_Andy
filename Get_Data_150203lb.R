...
lbFileInfo <- file.info("leaderboard.csv")
...
lbFileCreatdAt <- lbFileInfo[1,4]
lbFileAccessedAt <- lbFileInfo[1,6]
lbFileCreatdDay <- as.POSIXlt(lbFileCreatdAt)$yday
lbFileCreatdYear <- as.POSIXlt(lbFileCreatdAt)$year
...
if(thFileCreatdDay < (CurrentYear-thFileCreatdYear)*366+CurrentDay             # checks to see if any of the data files have not been downloaded today
   ...
   & lbFileCreatdDay < (CurrentYear-lbFileCreatdYear)*366+CurrentDay) {
  print("Use stored datamart file(s)? (y/n)")                                            # "n" lets you bypass downloading files to save time
  OldData <- readline()
}
...
#  if (lbFileCreatdDay < (CurrentYear-lbFileCreatdYear)*366 +CurrentDay) {
#    url <- "https://daggre_datamart_production.s3.amazonaws.com/leaderboard.json"
#    download.file(url,destfile="leaderboard.json.csv")
#  }
}
