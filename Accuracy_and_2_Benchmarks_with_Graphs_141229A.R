...
#title <- paste("Accuracy_and_Uniform_Benchmark ",tstop,".png", collapse="")      #Expanding title name
...
br <- seq(0,2,0.1)
title <- paste("Accuracy_and_Uniform_Benchmark ",tstop,".png", collapse="")
one <- hist(acqu,breaks=br)
two <- hist(acun,breaks=br)
count <- matrix(c(one$counts,two$counts), nrow=2,byrow=T)
colnames(count) <- one$mids
rownames(count) <- c("sciCast", "Uniform")
count <- as.table(count)
barplot(count,beside=T,col=c(rgb(1,0,0,0.5),rgb(0,0,1,0.5)),xlab="Brier Score",
        ylab="Number of Questions",cex.main=1,main=paste("Accuracy ",tStart," to ",tStop,sep=""))
mtext(paste("Brier Scores (Uniform) mean = ",round(acunm,3),sep=""),col=rgb(0,0,1,0.6), outer=T,side=3,line=-12.5,cex=0.75,font=1)
mtext(paste("Brier Scores (SciCast) mean = ",round(acqum,3),sep=""),col=rgb(1,0,0,0.6), outer=T,side=3,line=-10.5,cex=0.75,font=1)
mtext(paste('      Better on ',winaU,'% of questions',sep=''), outer=T,side=3,line=-4.5,cex=0.75,font=1,col=rgb(0,0,0,1))
mtext(paste('      Overall score improved ',impoU,'%',sep=''), outer=T,side=3,line=-5.5,cex=0.75,font=1,col=rgb(0,0,0,1))
dev.off()
...
title <- paste("Accuracy_and_Safe_Mode_Benchmark ",tstop,".png", collapse="")      #Expanding title name
...
barplot(count,beside=T,col=c(rgb(1,0,0,0.5),rgb(0,0,1,0.5)),xlab="Brier Score",
        ylab="Number of Questions",cex.main=1,main=paste("Accuracy ",tStart," to ",tStop,sep=""))
 mtext(paste("Brier Scores (ULinOP) mean = ",round(acopm,3),sep=""),sep=""),col=rgb(0,0,1,0.6), outer=T,side=3,line=-12.5,cex=0.75,font=1)
 mtext(paste("Brier SCores (SciCast) mean = ",round(acqum,2),sep=""),col=rgb(1,0,0,0.6), outer=T,side=3,line=-10.5,cex=0.75,font=1)
