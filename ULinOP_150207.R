...
start <- Sys.time() 
print("ULinOP calculations started")

#questionUserFrcstULinOP <- array(0,nrow=nsmfq, ncol=length(days))   ###not yet defined
...
#for (q in 19:19) {                    #tetsting only
 ...
 #print(clq[qiq==rsq[q]])
  ...
  if (clq[qiq==rsq[q]]=="ordered multinomial") {                      #if the question is ordered
...
#       tmp0[3] <- 0.3
        ...
        if (t<2) {                              # first forecast
         weight[w2] <- 1
          #tmp3 <- tmp0[or[t],1:length(tmp1)]     # excludes excess options   #### or calculated in previous section, same for w  ######
         tmp3 <- tmp0[or[t]]
         #(c(t,lw2, tmp0[or[t]],tmp3, weight[ltStart:ltStop]))
       }
       else {
...
print ("ULinOP calcuations Complete")
print(duration)